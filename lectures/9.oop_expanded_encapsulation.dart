...
//? 1. Реализовать паттерн синглтон
//* Часть 1
//* Создать класс логера (Logger)
//* Логгер должен иметь поле - префикс и мут (bool), оба паблик, но мут изменяемый.
//* Логгер должен иметь метод - log, который принимает сообщение и печатает его в консоль с помощью print и префикса
//* Логгер в конструкторе должен принимать префикс, мут по умолчанию false
//* Часть 2
//* Паттерн синглтон - это паттерн (подход к созданию класса, приём), который гарантирует,
//* что у класса есть только один экземпляр (объект).
//* Т.е. при каждом обращении к публичному конструктору класса должен возвращаться ОДИН и тот же объект.
//* Публичный конструктор ничего не принимает и ВСЕГДА, независимо от final или const при создании объекта возращает
//* Один и тот же объект (одинаковый hashcode)

//? 2. Реализовать дата класс для студента
//* Дата класс - это класс, который в основном базируется на хранении данных и эти данные НЕИЗМЕНЯЕМЫ
//* Класс в этом задании должен иметь
//* 1. Поля: имя, год постулпения, средний балл, список оценок (НЕ делать копирование при создании), возраст
//* Все поля именованные и обязательные в конструктор(е/ах)
//* 2. Константный конструктор
//* 3. Конструктор для создания студента из json
//* (json - Map<String, Object>, считается что Json идеальный - есть все поля и все типы верны)
//* 4. Метод copyWith для создания НОВОГО студента на основе старого (this) с возможностью изменить поля
//* (если поля переданы в аргументы метода), Если поля не переданы, то оставляем старые
//* 5. Метод toJson для преобразования студента в json (Map<String, Object>)
...
//* См. прошлое задание с микроволновками и переделайте его
//* Только теперь реализовать всё нужно в одном классе:
//* При создании микроволновки мы указываем бренд, новая микроволновка должна хранить modelNumber.
//* modelNumber для каждого бренда считается отдельно.
...
//? 5. Реализуйте класс «Кошелек» и «Валюта»
//* В класс "Кошелек" могут добавляться экземпляры дата-класса "Валюта" различного номинала.
//* Кошелек должен иметь возможность возвращать полную сумму денег,
//* У каждого кошелка максимальный лимит в 100 едениц валюты (не 100 объектов, а сумма номиналов),
//* последующее добавление валюты не должно приводить к увеличению суммы.
//* Правильно инкапсулируйте поля и методы, придумайте как можно использовать различные конструкторы.

//? 6. Реализуйте пространство имен для цветов
//* Создайте пространство имен для цветов (штук 6-8), в котором будут храниться константы цветов в HEX
//* Пространство имен должно быть доступно из любого места вашего приложения
//* Пространство имен должно называться Color
//* чтобы создать число в hex нужно написать 0xNUMBER, пример red = 0xFF0000
//* Пример использования пространства имен Color
//* print(Color.red); // 0xFF0000 (16711680)
//* print(Color.green); // 0x00FF00 (65280)
//* print(Color.blue); // 0x0000FF (255)
//* print(Color.black); // 0x000000 (0)
//* ... и д.р.
//* Пространство имен должно быть невозможно инициализировать, т.е. нельзя создать объект Color

//? 7. Реализуйте класс StrangeCipher
//* Класс имеет два поля - message и cipher (оба неизменяемые, типа String и публичные)
//* message передается через конструктор
//* cipher должен высчитываться из message следующим образом:
//* 1. message умножается на 500000 (увеличивается в 500000 раз)
//* 2. message разбивается на символы, каждый символ переводится в код символа (codeUnitAt(0))
//* 3. каждый код символа возводится в 12 степень (pow(el, 12).toInt())
//* 4. каждый результат переводится в 16-ричную систему счисления (toRadixString(16))
//* 5. полученные символы перемешиваются (shuffle)
//* 6. полученные символы объединяются в строку
//* 7. строка возвращается как результат
//? Запустите следующий код:
// print('start');
// final a = StrangeCipher('hello world');
// print('end');
//* Скорее всего вы увидите что после 'start' идет долгая пауза, а потом уже 'end'
//* Почему так происходит?
//* Допустим, что нам в коде никогда не придется использовать поле cipher, как можно ускорить создание объекта?
//? Сделайте так чтобы код:
// print('start');
// final a = StrangeCipher('hello world');
// print('end');
//* Выполнялся моментально, но при этом поле cipher существовало и было доступно для использования
